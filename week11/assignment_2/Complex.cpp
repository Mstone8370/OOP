...
Complex::Complex() {
     ...
     */
	this->value = std::make_pair(0.0, 0.0);
}
...
Number *Complex::add(Number * num) {
	...
	if (num->types() == INTEGER) {
		int operand_val = dynamic_cast<Integer*>(num)->val();
		set_val(std::get<0>(this->value) + operand_val, std::get<1>(this->value));
		delete(num);
		return this;
	} else if (num->types() == FLOAT) {
		float operand_val = dynamic_cast<Float*>(num)->val();
		set_val(std::get<0>(this->value) + operand_val, std::get<1>(this->value));
		delete(num);
		return this;
	} else if (num->types() == COMPLEX) {
		Complex* operand = dynamic_cast<Complex*>(num);
		set_val(std::get<0>(this->value) + std::get<0>(operand->val()), std::get<1>(this->value) + std::get<1>(operand->val()));
		delete(operand);
		return this;
	} else {
		return nullptr;
...
void Complex::set_val(double real, double imag) {
     ...
     * 이 때 make_pair함수를 이용
     */
	set_val(std::make_pair(real, imag));
}
