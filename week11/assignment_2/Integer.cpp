...
Number *Integer::add(Number * num) {
    ...
    if (num->types() == INTEGER) {
		this->value += dynamic_cast<Integer*>(num)->val();
		delete(num);
		return this;
    } else if (num->types() == FLOAT) {
		Float* operand = dynamic_cast<Float*>(num);
		operand->set_val(this->value + operand->val());
		delete(this);
		return operand;
    } else if (num->types() == COMPLEX) {
		Complex* operand = dynamic_cast<Complex*>(num);
		std::pair<double, double> operand_val = operand->val();
		float imag = std::get<1>(operand_val);
		operand->set_val(this->value + std::get<0>(operand_val), imag);
		delete(this);
		return operand;
    } else {
		return nullptr;
